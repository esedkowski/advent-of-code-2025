...
def make_circuits(distances, num_of_points):
    circuits = []
    for distance in distances:
        ...
        if len(circuits) == 1 and len(circuits[0]) == num_of_points:
            last_pair = (distance[0], distance[1])
            break
...
print(distances[:3])
sorted_dist = sort_distances(distances)
print(sorted_dist[:5])

circuits, last_pair = make_circuits(sorted_dist, len(points))
p1, p2 = last_pair[0], last_pair[1]
